...
var useragent = require('../'),
    ...
    fs = require('fs'); // run over the testcases, some might fail, some might not. This is just qu
// test to see if we can parse without errors, and with a reasonable amount
// of errors.


['testcases.yaml', 'static.custom.yaml', 'firefoxes.yaml', 'pgts.yaml'].forEach(async function (filename) {
  var testcases = (await fs_readFilePromise(__dirname + '/fixtures/' + filename)).toString(),
  ...
  testcases.forEach(function (test) {
    ...
    if (typeof test.user_agent_string !== 'string') return; // these tests suck as the test files are broken, enable this to have about
    // 40 more failing tests

    if (test.family.match(/googlebot|avant/i)) return; // attempt to parse the shizzle js based stuff
    ...
    exports[filename + ': ' + test.user_agent_string] = function () {
      ...
      assume(agent.family).equals(test.family); // we need to test if v1 is a string, because the yamlparser transforms
